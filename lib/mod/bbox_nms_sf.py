...
class BboxNMSLayerShareFeature(caffe.Layer):
	...
	def forward(self, bottom, top):
		...
		blob_org_rois = np.zeros((1,5), dtype=np.float32)
		...
		for batch_idx in xrange(len(num_of_rois)):
			...
			if self.last_clip_id != cur_clip_id:
				...
				if len(blob_rois) == 1 and np.count_nonzero(blob_rois) == 0:
					...
					blob_org_rois[0,:] = np.array([batch_idx,0,0,128,128])
					blob_label[0,0,0,0] = 0
				else:
					blob_rois = np.concatenate((blob_rois, np.array([[batch_idx,0,0,128,128],])))
					blob_org_rois = np.concatenate((blob_org_rois, np.array([[batch_idx,0,0,128,128],])))
				...
				if boxes_nms_xyxy.shape[0] != 0: 
					if len(self.mhi_buf) != 30:
						...
						if len(blob_rois) == 1 and np.count_nonzero(blob_rois) == 0:
							...
							blob_org_rois[0,:] = boxes_nms_xyxy_org[0][:]
							...
							if boxes_nms_xyxy.shape[0] > 1:
								...
								blob_org_rois = np.concatenate((blob_org_rois, boxes_nms_xyxy_org[1:]))
							...
							blob_org_rois = np.concatenate((blob_org_rois, boxes_nms_xyxy_org))
		...
		top[2].reshape(*(blob_org_rois.shape))
		top[2].data[...] = blob_org_rois.astype(np.float32,copy=False)
