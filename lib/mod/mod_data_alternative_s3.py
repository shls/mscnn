...
class ModDataLayer_alternative_s3(caffe.Layer):
	...
	def setup(self, bottom, top):
		...
		if (self.phase == caffe.TRAIN):
			...
			self._spatial_prefeature = ucfarg_cfg.TRAIN.SPATIAL_PREFEATURE_ROOT
			...
			self._spatial_prefeature = ucfarg_cfg.TEST.SPATIAL_PREFEATURE_ROOT
	...
	def forward(self, bottom, top):
		...
		for batch_index in xrange(self._imgs_per_batch):
			...
			spatial_pre_feature = np.asarray(np.load(os.path.join(self._spatial_prefeature, img_basename + self._data_extension)))

			if spatial_pre_feature.shape[1] != 512 or temporal_im.shape[0] !=1:
				print "image shape mismatch by Ls"
				raise

			blob_spatial_im[batch_index, :, :, :] = spatial_pre_feature[0][:]
